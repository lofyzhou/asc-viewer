...
import { ASCReader }                        from './asc-parser';
import { applyFilter, applySort, toWire }   from './asc-host';
import { getWebviewHtml, getNonce }         from './asc-webview';
import { WebviewMessage }                   from './asc-types';
import { parseDbcFile, DbcDatabase }        from './dbc-parser';

// ── Provider ──────────────────────────────────────────────────────────────────

export class ASCViewProvider implements vscode.CustomReadonlyEditorProvider {
  static readonly viewType = 'asc.viewer';
  ...
  private async parseFile(
    ...
    try {
      const reader   = new ASCReader(filePath);
